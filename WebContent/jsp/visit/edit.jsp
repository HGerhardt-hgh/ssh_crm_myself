...
<BODY>
				...
				<TR>
					...
					<TD vAlign=top width="100%" bgColor=#ffffff>
						<TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
							<TR>
								<TD class=manageHead>当前位置：客户拜访管理 &gt; 修改信息</TD>
							</TR>
							<TR>
								<TD height=2></TD>
							</TR>
	...
	<td>所属客户：</td>
	<td colspan="3">
		<select name="visitCustomer.custid">
			<c:forEach items="${customerlists }" var="customer">
				<!-- 
					当前联系人所属客户id 和所有客户id比较，如果相同 选中状态
				 -->
				 
				<option value="${customer.custid }" <c:if test="${customer.custid == visit.visitCustomer.custid }">selected</c:if>>${customer.custName }</option>
			</c:forEach>
		</select>
	...
	<td>所属用户：</td>
	<td colspan="3">
		<select name="visitUser.uid">
			<c:forEach items="${userLists }" var="user">
			<option value="${user.uid }" <c:if test="${user.uid==visit.visitUser.uid }">selected </c:if>>${ user.username}</option>
